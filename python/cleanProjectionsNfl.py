...
print 'load vend players'
with open('../projections/nfl/history.json') as data_file:    
...
for i, v in enumerate(players):
  ...
  vend_id = v['nfl_player_id']
  vend_nm = v['full_name']
  team_po = v['team_pos']
  vend_op = cleanTeamNameDK(v['opponent'])
  nfld_id = '';

  qgame = nfldb.Query(db).game(season_year=v['season_year'], week=v['week'], season_type='Regular', team=vend_op).as_games()
  if len(qgame) < 1:
    qgameid = '9999999999'
  else:
    qgameid = qgame[0].gsis_id

  if "D/ST" not in vend_nm:
    matches = nfldb.player_search(db, vend_nm, limit=15)
    optplayers = {}
    cnt_plyr = 0
    cnt_zero = 0
    print '---------------------------'
    ...
    for (player, dist) in matches:
      ...
      print str(cnt_plyr) + ') ' + str(vend_id) + ': Similarity score: %d, Player: %s' % (dist, player)
    if cnt_zero == 1:
      opt = '1'
    else:
      opt = raw_input('select the real...  ' + vend_nm + ' (' + str(team_po.replace(u'\xa0', u' ')) + '):')
    if opt == '0':
      fixPlayers.append({
        '1gsis_id': qgameid,
        '2player_id': 'none',
        '3vend_player_id': vend_id,
        '4full_name': vend_nm
      })
    else:
      rtnPlayers.append({
...
#Set up Worksheets
workbook = xlsxwriter.Workbook('excel/vend-players.xlsx')
